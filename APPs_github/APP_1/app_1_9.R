  ...
  report_data <- eventReactive(input$run_button, {
    ...
    if (input$report_frequency == "ANUAL") {
      report <- filtered_data %>%
        ...
        summarise(total_luaes = n(), .groups = 'drop')
    } 
    
    else if (input$report_frequency == "ACUMULADO") {
      report <- filtered_data %>%
        filter(
          estado_emision %in% c("EMITIDA", 
                                "EMITIDA PARCIAL",
                                "FINALIZADA", 
                                "OTORGADA", 
                                "OTORGADA PARCIAL") &
            ...
            (if (input$report_month != "TODOS") lubridate::month(fecha_otorgamiento) %in% as.numeric(input$report_month) else TRUE)
        ) %>%
